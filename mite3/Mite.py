...
class PatchedBinder(cef.JavascriptBindings):
    ...
    def SetProperty(self, name, value):
        ...
        allowed = self.IsValueAllowedRecursively(value)
    ...
    def IsValueAllowedRecursively(value, recursion=False):
        ...
        if valueType == list:
            for val in value:
                valueType2 = JavascriptBindings.IsValueAllowedRecursively(
                    val, True)
...
class Event():
    ...
    def __init__(self, *fargs):
        # P.S. I don't know what im doing.
        self.mite.pbug("Event {} called. Args: {}".format(
            self.function.__name__, fargs))
...
class Mite():
    ...
    def run(self, **settings):
        ...
        self.event_loop = settings[
            "event_loop"] if "event_loop" in settings else self.event_loop
    ...
    def start(self):
        ...
        self.pbug("Starting Application")
        self.j_funcs.append({"name": "miteErrorCallback",
                             "function": self.javascriptErrorCallback, 
                             "async": False})
        self.j_funcs.append({"name": "preReadyInit", 
                             "function": self.preReadyInitalize, 
                             "async": False})
        self.j_funcs.append({"name": "getcallback", 
                             "function": self.getcallback, 
                             "async": True})

        self.pbug("Appending Javascript Functions...")
        self._bindJS()
    ...
    async def preReadyInitalize(self):
        """Gets Executed before the app gets ready and 
    ...
    def _bindJS(self):
        ...
        bindings = PatchedBinder(bindToFrames=False, bindToPopups=False)
        ...
        for obj in self.j_obj:
            self.pbug("Loading {1} as {0}".format(
    ...
    def on_ready():
        ...
        def deco(f):
            Mite.j_funcs.append(
                {"name": "miteUIOnReady", "function": f, "async": True})
            return f
        return deco
